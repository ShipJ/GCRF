...
# BTS id -1 (missing) in the original data
vol = np.load('../data/civ/cdr_data/flows/total_vol.npy')
dur = np.load('../data/civ/cdr_data/flows/total_dur.npy')
bts_fn = '../data/civ/cdr_data/bts_xzero.csv'
if os.path.exists(bts_fn):
    ...
    bts_points = points_from_csv('../data/civ/cdr_data/bts_points.csv', x_col=1, y_col=2)
    # remove zero-flow towers from flow matrices
    ix = np.union1d(np.where(vol.sum(axis=0)>0)[0], np.where(vol.sum(axis=1)>0)[0])
    ix = ix[1:] # drop no tower index
    bts_ids = ix - 1
    bts_points = {bts_id:bts_points[bts_id] for bts_id in bts_ids}
    bts_coords = np.array([[p.x, p.y] for p in bts_points.values()])
...
rast_x = get_raster_data('../data/civ/geo/civ_pop/CIV14adjv1_utm30.tif')
...
D = squareform(pdist(bts_coords, 'euclidean'))
...
with open('../data/civ/total_vol.csv', 'w') as f:
...
bts_features = pd.concat([bts_features, vol_features, vol_res_features, dur_features], axis=1)
...
bts_features.to_csv('../data/civ/model_data/bts_12_months.csv', index=False, encoding='utf8')
...
for i, s in enumerate(grid_sizes):
    fn = '../data/civ/geo/civ_pop/CIV14adjv1_utm30_%d.tif' % s
    if not os.path.exists(fn):
        # transform original raster file to new grid size
        os.system('gdalwarp -tr %d %d -r average ../data/civ/geo/civ_pop/CIV14adjv1_utm30.tif %s' % (s,s,fn))
    rast, gt = get_raster_data(fn)
    # population aggregation is average so scale up accordingly
    scale = rast_x[0][rast_x[0]>0].sum() / rast[rast>0].sum()
    rast[rast>0] *= scale
    pop_rasts[i] = rast, gt

print "converting rasters to polygons..."
grids = [raster_to_polygons(rast) for rast in pop_rasts]
print "creating indices for grids..."
grid_idxs = [get_index(grid[0]) for grid in grids]
...
fn = '../data/civ/cluster_wealth_cells.csv'
if os.path.exists(fn):
    dhs = pd.read_csv(fn)
    cluster_coords = dhs[['long','lat']].values
else:
    dhs = pd.read_csv('../data/civ/cluster_wealth.csv')
...
for i in range(n_grids-1,-1,-1):
    ...
    nbrs, ds = point_within_distance(grids[i][0], bts_points.values(), r, grid_idxs[i])
    ws = np.array([w / w.sum() for w in 1 / (ds+1)])
    # create weights matrix
    print "computing weights matrix..."
    W = np.zeros((len(grid_points[i]), n_bts), dtype=np.float)
    for j in range(len(grid_points[i])):
        W[j, nbrs[j]] = ws[j]
    # create cell features dataframe
    feature_grids = pd.DataFrame({
        'id': range(len(grid_points[i])),
        'x': grid_points[i][:,0],
        'y': grid_points[i][:,1],
        'population': grid_pops[i]})
    print "computing cell features..."
    for col in bts_features.columns.tolist()[2:]:
        # compute cell features as weighted average of neighbouring BTS towers
        feature_grids[col] = np.apply_along_axis(lambda x: (x * bts_features[col]).sum(), 1, W)
    # save data
    print "saving data..."
    feature_grids.to_csv('../data/civ/model_data/cell_%d_12_months.csv' % grid_sizes[i], index=False, encoding='utf8')
print "fin."
...
#         fold += 1
    ...
    # grid_lags.to_csv('../data/civ/model_data/cells_%d_radius_lags_%d-%d.csv' %
...
#     print "\tsaving data..."
#     grid_lags = pd.concat(grid_lags)
#     grid_lags.to_csv('../data/civ/model_data/cells_%d_delaunay_%d-%d_lags.csv' %
#         (grid_sizes[i], n_train, n_test), index=False, encoding='utf8')
...
print "\tsaving data..."
grid_lags.to_csv('../data/civ/model_data/cells_%d_d2_lags_%d-%d.csv' %
    (grid_sizes[i], n_train, n_test), index=False, encoding='utf8')
