...
if __name__ == '__main__':
    ...
    group = 'None'
    models = ['NoOutliersNoTransform', 'OutliersNoTransformation',
              'NoOutliersLogLogTransform', 'OutliersLogLogTransform',
              'OutliersLogLinTransform', 'OutliersLinLogTransform']

    data = pd.DataFrame(pd.read_csv(PATH+'/final/%s/master_SL.csv' % country))

    for adm in np.setdiff1d(adms, []):
                ...
                for i in range(6):
                    ...
                    if group != 'None':
                        ...
                        if abs(pr[0]) > 0.4:
                            ...
                            plt.savefig('/Users/JackShipway/Desktop/Figures/%s/%s_%s_%s_%s.png' %
                                        (country, adm, k, j, models[i]), dpi=600)
                            plt.show()
